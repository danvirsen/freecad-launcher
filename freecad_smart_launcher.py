...
Linux and macOS desktop helper — not affiliated with the FreeCAD project.
"""
...
if IS_MAC:
    # FreeCAD releases ship as .dmg; the launcher installs them as .app bundles.
    FREECAD_BUNDLE_EXT = ".app"
    ...
    # so git / cmake / ninja / pixi / f3d are found.
    _extra_paths = [
        "/opt/homebrew/bin",
        "/usr/local/bin",
        "/opt/local/bin",
        os.path.expanduser("~/.pixi/bin"),
        "/Applications/F3D.app/Contents/MacOS",
        os.path.expanduser("~/Applications/F3D.app/Contents/MacOS"),
    ]
    _cur = os.environ.get("PATH", "").split(os.pathsep)
    os.environ["PATH"] = os.pathsep.join(_cur + [p for p in _extra_paths if p not in _cur])
else:
    FREECAD_BUNDLE_EXT = ".AppImage"
    MAC_ARCH = None


def pick_macos_dmg(assets):
    """Best .dmg for this Mac: matching CPU arch, highest macOS build target not newer than this OS."""
    try:
    ...
    for asset in assets:
        name = asset["name"]
        low = name.lower()
        if not (low.endswith(".dmg") and "macos" in low and MAC_ARCH in name):
            continue
        m = re.search(r"macos(\d+)", low)  # weeklies: macOS11-arm64, macOS15-arm64, ...
        min_os = int(m.group(1)) if m else 0
        if min_os <= os_major and min_os > best_min:
            best, best_min = asset, min_os
    return best


def strip_bundle_ext(filename):
    return filename[: -len(FREECAD_BUNDLE_EXT)] if filename.endswith(FREECAD_BUNDLE_EXT) else filename


def freecad_executable(bundle_path):
    """Path to execute for an installed version (the AppImage itself, or the binary inside the .app)."""
    if IS_MAC and bundle_path.endswith(".app"):
        return os.path.join(bundle_path, "Contents", "MacOS", "FreeCAD")
    return bundle_path


def version_root(path):
    """Inverse of freecad_executable(): the .app bundle for a binary inside it, else path."""
    if IS_MAC:
...
def remove_installed_version(path):
    root = version_root(path)
...
def child_env(base=None):
    """Environment for programs we start, without variables injected by a frozen (PyInstaller) build."""
...
def freecad_config_dir():
    if IS_MAC:
        return os.path.join(os.path.expanduser("~"), "Library", "Application Support", "FreeCAD")
    return os.path.join(os.path.expanduser("~"), ".config", "FreeCAD")


def install_app_from_dmg(dmg_path, target_app, status_cb=None):
    """Mount a FreeCAD .dmg, copy its .app to target_app, unmount."""
    mount_point = tempfile.mkdtemp(prefix="freecad_dmg_")
    staging = target_app + ".installing"
    attached = False
    try:
        if status_cb:
            status_cb("Mounting disk image...")
        subprocess.run(
        ...
        if not apps:
            raise IOError("No .app bundle found in the disk image.")
        apps.sort(key=lambda f: (0 if f.lower().startswith("freecad") else 1, f))
        if status_cb:
            status_cb("Copying FreeCAD.app...")
        if os.path.exists(staging):
            shutil.rmtree(staging, ignore_errors=True)
        # ditto keeps symlinks, permissions and the code signature intact
        subprocess.run(["ditto", os.path.join(mount_point, apps[0]), staging],
                       check=True, timeout=1800)
        subprocess.run(["xattr", "-dr", "com.apple.quarantine", staging],
                       stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL)
        os.replace(staging, target_app)
    finally:
        if attached:
            r = subprocess.run(["hdiutil", "detach", mount_point, "-quiet"],
                               stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL)
            if r.returncode != 0:
...
try:
    # python.org / frozen macOS builds often have no system CA store configured
    import certifi
...
from PySide6.QtWidgets import (
    ...
    QAbstractItemView, QStyleFactory, QGraphicsDropShadowEffect, QLayout
)
...
def disable_freecad_start_page():
    base = freecad_config_dir()
    cfg_paths = [os.path.join(base, "user.cfg")]
    if os.path.isdir(base):
        cfg_paths += [
            os.path.join(base, d, "user.cfg") for d in os.listdir(base)
            if d.startswith("v") and os.path.isfile(os.path.join(base, d, "user.cfg"))
        ]
    for cfg_path in cfg_paths:
        if os.path.exists(cfg_path):
            _disable_start_page_in(cfg_path)


def _disable_start_page_in(cfg_path):
    try:
...
def run_freecad_console_script(app_path, script_path, timeout=60):
    ...
    app_path = freecad_executable(app_path)
    attempts = [
    ...
    for cmd in attempts:
        try:
            result = subprocess.run(
                ...
                timeout=timeout,
                env={**child_env(), "QT_QPA_PLATFORM": "offscreen"},
            )
            if result.returncode == 0:
                return True
...
def export_fcstd_to_temp_cad(app_path, filepath, timeout=120):
    ...
    if not app_path or not os.path.isfile(freecad_executable(app_path)):
        return None
...
class StatsDialog(QDialog):
    ...
    def _populate_tree(self, stats_data):
        ...
        for ver_name, data in stats_data.items():
            ...
            clean_name = strip_bundle_ext(ver_name)
...
class InfoDialog(QDialog):
    def __init__(self, parent, is_dark=True):
        ...
        wiki = "https://wiki.freecad.org/Compile_on_MacOS" if IS_MAC else "https://wiki.freecad.org/Compile_on_Linux"
        link = QLabel(f'<a href="{wiki}">{wiki}</a>')
...
class FreeCADLauncher(QMainWindow):
    ...
    def format_version_name(self, filename, is_weekly):
        base = strip_bundle_ext(filename)
        if is_weekly:
    ...
    def build_ui(self):
        ...
        # Action buttons — isolated strip with fixed height (no overlap on resize)
        btn_wrap = QWidget()
        btn_wrap.setFixedHeight(36)
        ...
        btn_menu = QPushButton("Show in Finder" if IS_MAC else "Desktop entry")
        btn_menu.setObjectName("accent")
        btn_menu.setFixedHeight(28)
        btn_menu.setToolTip(
            "Reveal the selected version in Finder (drag it to the Dock to pin it)"
            if IS_MAC else
        ...
        pr_row.addWidget(self.btn_compile_pr)
        ...
        pr_row.addWidget(self.btn_stop_pr)
        ...
        pr_row.addWidget(self.btn_just_launch_pr)
        pr_row.addStretch()
        b2_lay.addLayout(pr_row)

        self.chk_pixi = QCheckBox("Build with pixi")
        self.chk_pixi.setToolTip(
            "Use pixi run configure/build instead of system cmake. "
            "Enable only if you compile FreeCAD with pixi. "
            "A pixi.toml in the sources is normal upstream and does not force pixi."
        )
        self.chk_pixi.setChecked(self.var_use_pixi_build)
        self.chk_pixi.toggled.connect(self.toggle_use_pixi_build)
        b2_lay.addWidget(self.chk_pixi)

        info_row = QHBoxLayout()
        self.lbl_pr_info = QLabel("Title: -\nAuthor: -")
        self.lbl_pr_info.setObjectName("muted")
        info_row.addWidget(self.lbl_pr_info, 1)
        self.lbl_pr_status = QLabel("Ready.")
        self.lbl_pr_status.setObjectName("muted")
        info_row.addWidget(self.lbl_pr_status)
        b2_lay.addLayout(info_row)
        left.addWidget(b2)
        ...
        self.lbl_status = QLabel("Searching for updates on GitHub...")
        self.lbl_status.setObjectName("muted")
    ...
    def get_pr_exe_path(self, src_dir):
        ...
        if IS_MAC:
            candidates += [
                os.path.join(src_dir, "build", sub, "bin", "FreeCAD.app", "Contents", "MacOS", "FreeCAD")
                for sub in ("debug", "release", "")
            ]
        for exe_path in candidates:
            if os.path.isfile(exe_path):
                bin_dir = os.path.dirname(version_root(exe_path))
                return os.path.dirname(bin_dir), exe_path
    ...
    def refresh_local_versions(self):
        ...
        if os.path.exists(install_dir):
            appimages = [f for f in os.listdir(install_dir) if f.endswith(FREECAD_BUNDLE_EXT)]
    ...
    def fetch_github_releases(self):
        try:
            ...
            for rel in releases:
                ...
                assets = rel["assets"]
                if IS_MAC:
                    asset = pick_macos_dmg(assets)
                    assets = [asset] if asset else []
                for asset in assets:
                    name = asset["name"]
                    if IS_MAC:
                        name = name[:-4] + ".app"  # installed bundle name
                    elif not (name.endswith(".AppImage") and ("x86_64" in name or "conda" in name)):
                        continue
    ...
    def _available_updates(self):
        ...
        if os.path.isdir(install_dir):
            local_names = {f for f in os.listdir(install_dir) if f.endswith(FREECAD_BUNDLE_EXT)}
    ...
    def _on_update_download_finished(self, filename):
        ...
        if old_path and os.path.exists(old_path) and os.path.abspath(version_root(old_path)) != os.path.abspath(new_path):
            msg = (
            ...
            if reply == QMessageBox.Yes:
                ...
                try:
                    remove_installed_version(old_path)
                    self.refresh_local_versions()
    ...
    def download_version(self, filename, from_download_dialog=False):
        ...
        def downloader():
            ...
            try:
                ...
                if IS_MAC:
                    self.sig_download_status.emit(f"Installing {filename}...")
                    install_app_from_dmg(tmp_path, target_path)
                    os.remove(tmp_path)
                else:
    ...
    def change_src_folder(self):
        folder = pick_existing_directory(
            ...
            "Select FreeCAD Source Directory",
        )
        if folder:
            self.config_data["src_folder"] = folder
            self.src_folder_edit.setText(folder)
            self.save_config()
        ...
        def watch_for_fuse_failure():
            ...
            try:
                if process.returncode == 0:
                    return
                if fuse_signature_seen and IS_LINUX:
    ...
    def create_desktop_entry(self):
        """Create a .desktop entry for the selected AppImage (macOS: reveal the .app in Finder)."""
        ver = self._selected_version()
        if not ver:
            QMessageBox.warning(self, "Warning", "Please select a FreeCAD version first.")
            return
        if IS_MAC:
            subprocess.Popen(["open", "-R", ver["path"]])
            return
                ...
                if display and IS_LINUX:
                    ...
                    if os.path.isfile(desk):
                        try:
                            ...
                            pass
                remove_installed_version(ver["path"])
                self.refresh_local_versions()
            except Exception as e:
