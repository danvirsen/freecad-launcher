...
class FreeCADLauncher(QMainWindow):
    ...
    def get_pr_exe_path(self, src_dir):
        build_dir = os.path.join(src_dir, "build")
        exe_path = os.path.join(build_dir, "debug", "bin", "FreeCAD")
        if not os.path.exists(exe_path):
            exe_path = os.path.join(build_dir, "bin", "FreeCAD")
    ...
    def build_and_run_pr(self):
        ...
        missing = self._check_build_dependencies()
        if missing:
            QMessageBox.critical(
                self, "Missing dependencies",
                "The following tools are required but not found in PATH:\n\n"
        ...
        def task():
            try:
                ...
                cache_file = os.path.join(build_dir, "CMakeCache.txt")
                cmake_configure_cmd = ["cmake", "-B", build_dir, "-S", src_dir, "-DCMAKE_BUILD_TYPE=Debug"]
                if shutil.which("ninja"):
                    cmake_configure_cmd += ["-G", "Ninja"]
                if shutil.which("ccache"):
                    cmake_configure_cmd += [
                ...
                try:
                    ...
                    if os.path.exists(cache_file) and ("CMAKE_BUILD_TYPE" in str(cmake_err) or "cache" in str(cmake_err).lower()):
                        log_build("CMake cache conflict — wiping build dir and reconfiguring")
                        shutil.rmtree(build_dir, ignore_errors=True)
                        run_step(cmake_configure_cmd)
                    else:
                ...
                self.sig_pr_status.emit(f"Compiling PR #{pr_num}... (0%)")
                run_step(["cmake", "--build", build_dir, "-j", str(os.cpu_count() or 4)], progress_cb=update_build_progress)
    ...
    def launch_compiled_pr_instance(self, exe_path, pr_num):
        ...
        if self.var_autoclose:
            ...
            def wait_and_log():
                pr_process.wait()
                duration = int(time.time() - pr_start_time)
                if duration > 5:
                    if stat_key not in self.stats_data:
                        self.stats_data[stat_key] = {"time_sec": 0, "launches": 0}
                    self.stats_data[stat_key]["time_sec"] += duration
                    self.stats_data[stat_key]["launches"] += 1
                    self.save_stats()
                    self.sig_footer.emit()
                if vanilla_dir:
                    shutil.rmtree(vanilla_dir, ignore_errors=True)
